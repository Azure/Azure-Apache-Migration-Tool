...
{    
    my $auqRetVal;
    ($auqRetVal,$localConfFilePath) = auth_main($ARGV[0],$ARGV[1]);        # AUQ module functionality    
    if(!($auqRetVal))
    {
        &DeleteWorkingFolder();
    ...
    &pars_Generate2D(&utf_getCompleteFilePath(FILE_CONF_ALL), &utf_getCompleteFilePath(FILE_RECOVERY), $RecoveryMode,$ARGV[2]);

	if(! &pars_IsNoSiteSelected()) { &pars_UploadPublishSettingsAllSites($ARGV[3]);}
